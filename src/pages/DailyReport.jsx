      ...
      try {
        const response = await axios.post('http://localhost:1337/graphql', {
          ...
          query getSchedulesForUser($id: ID!) {
            schedules(
              filters: { teaching: { users_permissions_user: { id: { eq: $id } } } }
            ) {
              data {
                id
                attributes {
            ...
            <tbody>
              {data.map((item, index) => (
                <tr key={item.id}>
                  ...
                  <td className="border px-4 py-2 h-[8vh]">{item.attributes.hari+ ", " + periodToJadwal[item.attributes.jadwal]}</td>
                  ...
                  <td className="border px-4 py-2 h-[8vh] flex justify-center items-center">
                    <Link to={`/daily-report-teacher-schedule/${item.id}`}>
